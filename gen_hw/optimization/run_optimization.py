...
from criteria.L1_loss import L1_Loss
...
def main(args):
    ...
    ori, _ = g_ema([latent], input_is_latent=True, randomize_noise=False)
    res = ori

    clip_loss = CLIPLoss(args)
    if args.id_lambda > 0:
        id_loss = IDLoss(args).cuda()
    # l1_loss = L1_Loss(args).cuda()
    ...
    for i in pbar:
        ...
        if args.mode == "edit":
            # l_1_loss = l1_loss(img_gen, ori)
            l2_loss = ((latent_code_init - latent) ** 2).sum()
            loss = c_loss + args.l2_lambda * l2_loss
            if args.id_lambda > 0:
                i_loss, _ = id_loss(img_gen, ori)
                loss += args.id_lambda * i_loss
            if args.l1_lambda > 0:
                l_1_loss = torch.norm(latent_code_init - latent, p=1).cuda()
                loss += args.l1_lambda * l_1_loss
        else:
            loss = c_loss
    ...
    # out_dir = "results/ffhq/" + args.description.replace(' ', '_') + "/"
    # if not os.path.exists(out_dir):
    #     os.makedirs(out_dir)
    # torchvision.utils.save_image(img_gen, out_dir + str(args.image_id) + ".jpg", normalize=True, range=(-1, 1))
    return final_result, res, img_gen
